...
@overload
def _get_header_and_column_types(draw, header: is_seq, columns: is_none):
...
def csv(draw, header=None, columns=None, lines=None, dialect="excel"):
    ...
    header = header_param or [
        "col_{}".format(i) for i in range(len(rows[0]))
    ]  # placeholder header for DictWriter
