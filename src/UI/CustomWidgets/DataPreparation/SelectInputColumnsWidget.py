...
class SelectInputColumnsWidget(iw.VBox):

    name = "Select Input Columns"
    ...
    def _on_layer_dropdown_value_change(self, change: Any) -> None:
        ...
        self.layer_fullness_status.value = f"Layer fullness: {self.data_manager.columns_per_layer.get(change['new'])}/{self.model_manager.input_shapes.get(change['new'])}"
    ...
    def _on_add_columns_button_clicked(self, _) -> None:
        ...
        with self.columns_status:
            ...
            self.layer_fullness_status.value = f"Layer fullness: {self.data_manager.columns_per_layer.get(layer)}/{self.model_manager.input_shapes.get(layer)}"
    ...
    def _on_model_instantiated(self) -> None:
        ...
        layers = list(self.model_manager.input_layers)
    ...
    def _on_output_columns_added(self) -> None:
        ...
        self.columns_select.options = [
            ...
            for item in self.columns_select.options
            if item
            not in it.chain.from_iterable(self.data_manager.output_columns.values())
        ]
